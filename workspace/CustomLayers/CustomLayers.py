...
class BinDense(Layer):
    """Just your regular Binarized densely-connected NN layer.
    ...
    def __init__(self, units,
        ...
        super(BinDense, self).__init__(**kwargs)
        self.units = units
    ...
    def call(self, inputs):
        ...
        output = K.dot(inputs, bin_kernel)
        if self.use_bias:
    ...
    def get_config(self):
        ...
        base_config = super(BinDense, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

class MultiDense(Layer):
    """Just your regular Multibit-quantized densely-connected NN layer.
    `Dense` implements the operation:
    `output = activation(dot(input, kernel) + bias)`
    where `activation` is the element-wise activation function
    passed as the `activation` argument, `kernel` is a weights matrix
